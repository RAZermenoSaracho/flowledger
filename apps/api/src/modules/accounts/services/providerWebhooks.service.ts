...
export async function handleProviderWebhook(input: {
  ...
  body: unknown;
}): Promise<ProviderWebhookHandlingResult> {
  const providerKey = input.providerParam as ProviderKey;
  const provider = getProvider(providerKey);
  const rawHeaders = sanitizeWebhookHeaders(input.headers);
  const rawBodyText = rawBodyString(input.rawBody);

  if (providerKey !== "syncfy") {
    const eventId = `generic:${randomUUID()}`;
    const recordedEvent = await prisma.providerWebhookEvent.create({
      data: {
        provider: input.providerParam,
        providerEventId: eventId,
        eventName: "webhook",
        rawPayload: toJsonValue(input.body),
        rawHeaders,
        rawBody: rawBodyText,
        status: "received"
      }
    ...
    return { kind: "generic", result };
  }

  const signatureHeader = findSyncfySignatureHeader(input.headers);
  ...
  console.info("[PROVIDER WEBHOOK] Syncfy signature verification", {
    ...
    contentType: getHeaderString(input.headers["content-type"]),
    hasContentEncoding: Boolean(
      getHeaderString(input.headers["content-encoding"])
    )
  });
  ...
  if (signatureVerification === "invalid") {
    await recordInvalidWebhook({
      ...
      rawBody: {
        ...
        parsedJson: input.body !== undefined
      },
      rawHeaders,
      rawBodyText: "",
  ...
  const parsedWebhook = syncfyWebhookSchema.safeParse(input.body);
